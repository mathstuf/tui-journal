...
impl DataProvider for VjournalDataProvide {
    async fn load_all_entries(&self) -> anyhow::Result<Vec<Entry>> {
        ...
        for file_path in &files {
            ...
            for sub in &vcal.subcomponents {
                ...
                let id = state.assign_id(uid, file_path, None);
    ...
    async fn restore_entry(&self, entry: Entry) -> Result<Entry, ModifyEntryError> {
        ...
        if state.id_to_location.contains_key(&entry.id) {
    ...
    async fn update_entry(&self, entry: Entry) -> Result<Entry, ModifyEntryError> {
        ...
        let (file_path, uid) = {
            ...
            let loc = state.id_to_location.get(&entry.id).ok_or_else(|| {
...
impl VjournalDataProvide {
    async fn write_entry(
        ...
        let mut state = self.state.lock().await;
        let id = state.assign_id(&uid, &file_path, id);
